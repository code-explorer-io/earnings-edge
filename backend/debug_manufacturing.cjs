...
console.log('DEBUGGING: Manufacturing mentions in Apple earnings transcripts');
console.log('='.repeat(80));
...
transcripts.forEach((transcript, idx) => {
  ...
  searchTerms.forEach(term => {
    ...
    if (count > 0) {
      ...
      // Show context for each match (but limit to first 3 to avoid overwhelming output)
      const matchesToShow = matches.slice(0, 3);
      matchesToShow.forEach((matchIndex, i) => {
      ...
      if (matches.length > 3) {
        console.log(`\n... and ${matches.length - 3} more mentions (showing first 3 only)`);
      }
...
// Additional check: Search for any word starting with "manufactur"
console.log('\n\n');
console.log('='.repeat(80));
console.log('BONUS CHECK: Any words starting with "manufactur"');
console.log('='.repeat(80));

transcripts.forEach(transcript => {
  const text = transcript.transcript || '';
  const regex = /\bmanufactur\w*/gi;
